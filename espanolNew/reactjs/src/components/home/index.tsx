...
const Home = () => {
  const [espanolContent, setEspanolContent] = useState<string>('');
  const [rotateBCK, setRotateBCK] = useState<string>('');
  const [rotateBCK2, setRotateBCK2] = useState<string>('');
  const [rotateBCK3, setRotateBCK3] = useState<string>('');
  const [matLinks, setMatLinks] = useState<string>('');
  const [servLinks, setServLinks] = useState<string>('');
  const [recLinks, setRecLinks] = useState<string>('');

  const axioURL = (url: string, stateVar: any) => {
    const axObjFunc = () => {
      axios.get(url)
        .then((res) => {
          stateVar(res.data);
  ...
  axioURL('http://www.scsk12.org/espanolNew/phpMySQLAPI.php', setEspanolContent);
  axioURL('http://www.scsk12.org/espanolNew/rotateBckAPI.php', setRotateBCK);
  axioURL('http://www.scsk12.org/espanolNew/rotateBckAPI2.php', setRotateBCK2);
  axioURL('http://www.scsk12.org/espanolNew/rotateBckAPI3.php', setRotateBCK3);
  axioURL('http://www.scsk12.org/espanolNew/matAPI.php', setMatLinks);
  axioURL('http://www.scsk12.org/espanolNew/servAPI.php', setServLinks);
  axioURL('http://www.scsk12.org/espanolNew/recAPI.php', setRecLinks);
  ...
  const strArr = (api: string, log: string) => {
    const apiSplit = api.split(' ');
    const apiObj = apiSplit.join(' ')
      .replace(/\n{2,}/, '')
      .replace(/\n{3,}/g, '')
      .replace(/\s{3,}/g, ',  ')
      .split(',  ');
  ...
  const objRotateBCK = strArr(rotateBCK, 'ROTATE IMAGE DATA: ');
  const objRotateBCK2 = strArr(rotateBCK2, 'ROTATE IMAGE DATA 2: ');
  const objRotateBCK3 = strArr(rotateBCK3, 'ROTATE IMAGE DATA 3: ');
  const objMatLinks = strArr(matLinks, 'MAT LINKS DATA: ');
  const objServLinks = strArr(servLinks, 'MAT LINKS DATA: ');
  const objRecLinks = strArr(recLinks, 'REC LINKS DATA: ');
      ...
      <article>
        ...
        <section id="bck" className="large-12 small-12 espanolCopy">
...

          {
            ...
            //   <li key={images}>
            //     <a href={`${images.split(/\s{2,}/g)[0]}`} target="_blank" rel="noreferrer">
        ...
        <section id="welcomeWrap" className="large-12">
          ...
          <article className="espanolCopy">
            <section className="headingThree">
              <p className="copy">
                <a href="http://www.scsk12.org/espanolNew" target="_blank" rel="noreferrer">
                  Haga clic aquí
                </a>
                .
              </p>
      ...
      <article className="large-12">
        ...
        <section className="large-12 espanolCopy">
          ...
          <p className="copy">
            {obj[5]}
            &nbsp;
            <a href="http://www.scsk12.org/espanolNew">Haga clic aquí</a>
            .
          </p>
        </section>
