...
class Header extends React.Component {
  ...
  componentDidMount() {
    ...
    window.addEventListener('scroll', (event) => {
      windowScrollTo(document.querySelectorAll('html, body'));

      if (window.scrollTop > 0) {
        logoH1.style.cssText = 'margin-top: -70px; opacity: 0; transition: margin-top 300ms, opacity 300ms ease-in-out;';
        setTimeout(() => {
    ...
    window.addEventListener('scroll', (event) => {
      ...
      if (window.scrollTop === 0) {
        ...
        logoH1.style.cssText = 'margin-top: 0px; opacity: 1; transition: margin-top 500ms, opacity 500ms ease-in-out;';
      }
    });
